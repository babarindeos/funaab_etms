<x-admin-layout>
    <div class="container mx-auto">
        ...
        <section class="mb-8">
                <div>
                    <form  action="{{ route('admin.departments.hods.update', ['department'=>$department->id, 'hod'=>$department->hod->id])}} " method="POST" class="flex flex-col mx-auto w-[90%] items-center justify-center">
                        @csrf

                        

                        <div class="flex flex-col w-[80%] md:w-[60%] py-2 md:py-4" style="font-family:'Lato'; font-size:18px; font-weight:400;">
                            <h2 class="font-semibold text-xl py-1" >Edit HOD</h2>
                            ...
                            <select name="staff" class="border border-1 border-gray-400 bg-gray-50
                                                                    ...
                                                                    <option value=''>-- Select Staff --</option>
                                                                        @foreach($staff as $person)
                                                                            <option class='py-4' value="{{$person->user->id}}" @if($person->user_id == $department->hod->user_id) selected @endif>
                                                                                {{$person->surname}} {{$person->firstname}} ({{$person->fileno}}) 
                                                                            </option>
                                                                        @endforeach                                                                       
                                                                    </select>
                        ...
                        <div class="flex flex-col border-red-900 w-[80%] md:w-[60%] mt-4">
                            <button type="submit" class="border border-1 bg-gray-400 py-4 text-white 
                                           hover:bg-gray-500
                                           rounded-md text-lg" style="font-family:'Lato';font-weight:500;">Update HOD</button>
                        </div>
