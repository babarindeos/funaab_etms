<x-admin-layout>
    <div class="flex flex-col w-full mx-auto">
        ...
        <section class="mb-8">
                <div>
                    <form  action="{{ route('admin.cells.update', ['cell' => $cell->id])}} " method="POST" class="flex flex-col mx-auto w-[90%] items-center justify-center">
                        @csrf

                        

                        <div class="flex flex-col w-[80%] md:w-[60%] py-2 md:py-4" style="font-family:'Lato'; font-size:18px; font-weight:400;">
                            <h2 class="font-semibold text-xl py-1" >Edit Cell</h2>
                            Select Cell Type, Parent and provide name and code
                        </div>
                            ...
                            <select name="type" class="border border-1 border-gray-400 bg-gray-50
                                                                    ...
                                                                    <option value=''>-- Select Cell Type --</option>
                                                                        @foreach($cell_types as $cell_type)
                                                                            <option class='py-4' value="{{$cell_type->id}}" @if($cell_type->id == $cell->cell_type_id) selected  @endif)>{{$cell_type->name}} </option>
                                                                        @endforeach                                                                    
                                                                    </select>
                        ...
                        <div class="flex flex-col border-red-900 w-[80%] md:w-[60%] py-2">
                            ...
                            <select name="parent" class="border border-1 border-gray-400 bg-gray-50
                                                                    ...
                                                                    <option value=''>Top Level Parent</option>
                                                                        @foreach($cell_lists as $cell_item)
                                                                            <option class='py-4' value="{{$cell_item->id}}" @if($cell->parent == $cell_item->id) selected @endif >{{$cell_item->name}} ({{$cell_item->cell_type->name}}) </option>
                                                                        @endforeach                                                                    
                                                                    </select>
                        ...
                        <div class="flex flex-col border-red-900 w-[80%] md:w-[60%] mt-4">
                            <button type="submit" class="border border-1 bg-gray-400 py-4 text-white 
                                           hover:bg-gray-500
                                           rounded-md text-lg" style="font-family:'Lato';font-weight:500;">Update Cell</button>
                        </div>
                        
                    </form><!-- end of new department form //-->
                <div>
        </section>
        <!-- end of new Cell form //-->
